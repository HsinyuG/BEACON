...
# frozen evaluator copied from the /workspace/xtuner/evaluation/metrics/external_metric.py
...
# wrapper
class SingleFrameEvaluator:
    def __init__(self, npz_path: str, dataset_root: str):
        self.npz_path = npz_path
        self.dataset_root = dataset_root
        self.metric_evaluator = ExternalMetric(collect_device="cpu")

    @staticmethod
    def _load_predictions(npz_path: str):
        d = np.load(npz_path, allow_pickle=True)
        if "token" not in d:
            raise KeyError(f"[ERR] NPZ missing key 'token': {npz_path}")
        if "p_pred" not in d:
            raise KeyError(f"[ERR] NPZ missing key 'p_pred': {npz_path}")

        tokens = d["token"]
        p_pred = d["p_pred"].astype(np.float32)  # (N,3)

        pred_by_token = {}
        dup = 0
        for i in range(len(tokens)):
            tok = str(tokens[i])
            if tok in pred_by_token:
                dup += 1
                continue
            pred_by_token[tok] = p_pred[i]

        if dup > 0:
            print(f"[WARN] duplicate tokens in predictions NPZ: {dup} (kept first occurrence)")

        return pred_by_token, set(pred_by_token.keys())

    @staticmethod
    def _as_bool_tensor(x: np.ndarray) -> torch.Tensor:
        # Supports uint8/bool; make CPU bool tensor.
        if x is None:
    ...
    def evaluate(self):
        pred_by_token, pred_token_set = self._load_predictions(self.npz_path)
        ...
        remaining = set(pred_token_set)  # for early-exit when all preds are evaluated
        missing_pred = 0
        total = 0
        matched = 0
        ...
        for sample in tqdm(loader, desc="Evaluating", unit="sample"):
            total += 1
            tok = str(sample.get("token", ""))
            if tok == "":
                raise ValueError("[ERR] sample has empty token")

            # skip samples with no prediction (partial NPZ supported)
            if tok not in pred_by_token:
                missing_pred += 1
                continue
            ...
            # ---- dataset fields (B=1) ----
            A_np = sample.get("traversable_mask", None)
            B_np = sample.get("visible_mask", None)
            C_np = sample.get("affordance_mask", None)
            if A_np is None or B_np is None or C_np is None:
                raise KeyError(f"[ERR] missing masks in dataset sample token={tok}")

            A = self._as_bool_tensor(A_np).unsqueeze(0)  # (1,H,W)
            B = self._as_bool_tensor(B_np).unsqueeze(0)
            C = self._as_bool_tensor(C_np).unsqueeze(0)

            p_goal = self._as_f32_tensor(sample["p_goal"]).view(1, 3)  # (1,3)

            depths = sample.get("depths", None)
            cam2imgs = sample.get("cam2imgs", None)
            cam2egos = sample.get("cam2egos", None)
            if not isinstance(depths, dict) or not isinstance(cam2imgs, dict) or not isinstance(cam2egos, dict):
                raise TypeError(f"[ERR] expected depths/cam2imgs/cam2egos as dicts in sample token={tok}")
            ...
            # ---- predictions (B=1, K=1) ----
            p_pred = pred_by_token[tok]  # (3,)
            pred_xy = torch.tensor(p_pred[:2], dtype=torch.float32)  # (2,)
            pred_aff = torch.zeros_like(A[0], dtype=torch.bool)      # (H,W) dummy

            data_samples = [{"pred_xy": pred_xy, "pred_aff": pred_aff}]

            self.metric_evaluator.process(data_batch, data_samples)

            # early-exit: evaluated all predictions in the NPZ
            if len(remaining) == 0:
                break
...
if __name__ == "__main__":
    # Example invocation. Point these at your own exported shards / prediction npz.
    npz_path = "single_frame_results_val_unseen_first2shards.npz"
    dataset_root = "/path/to/processed_val_unseen_for_baselines/N400_size200_maxshards2"

    evaluator = SingleFrameEvaluator(npz_path=npz_path, dataset_root=dataset_root)
    metrics = evaluator.evaluate()
    print("Returned metrics dict keys:", list(metrics.keys()))