...
finetune_head_lr_mult = 1.0 if not is_stage2 else 1.0
...
if 'mini' in pkl_path:
    ...
    mini_tag = 'overfit'
work_dir = f'work_dir/{log_name}_PE3D_{use_pe3d}_batch{batch_size}x{int(accumulative_counts)}_lr{lr}x{int(finetune_head_lr_mult)}_epoch{num_epoch_budget}_{mini_tag}'
...
vis_backends = [
    ...
    dict(type='WandbVisBackend', init_kwargs=dict(project='thesis_hpc', name=f'{log_name}_PE3D_{use_pe3d}_batch{batch_size}x{accumulative_counts}_lr{lr}x{int(finetune_head_lr_mult)}_epoch{num_epoch_budget}_{mini_tag}')),
]
...
if output_mode == "Affordance":
    if _IS_TEST:
        ...
        if not resume:
            if is_stage2:
                load_from = f'{BEACON_WORK_ROOT}/work_dir/QA_PE3D_True_batch4x2_lr3e-05_epoch1_/iter_18779.pth'
            else:
