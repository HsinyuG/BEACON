...
from xtuner.evaluation.metrics.external_metric import ExternalMetric_QA
...
output_mode = "VQA"  # "RoboPoint" or "Affordance" or "VQA"
...
if 'mini' in pkl_path:
    ...
    training_set = 75116  # filtered effective train set size target visible is 76419, all_navigable is 92802, has npz is 133531, total is 240329
...
use_pe3d = True
method_variant = 'mlpxy' # 'mlpxy' or 'catbev'
...
if 'mini' in pkl_path:
    ...
    mini_tag = 'overfit'
work_dir = f'work_dir/{log_name}_PE3D_{use_pe3d}_batch{batch_size}x{accumulative_counts}_lr{lr}_epoch{num_epoch_budget}_{mini_tag}'
...
vis_backends = [
    ...
    dict(type='WandbVisBackend', init_kwargs=dict(project='thesis_hpc', name=f'{log_name}_PE3D_{use_pe3d}_batch{batch_size}x{accumulative_counts}_lr{lr}_epoch{num_epoch_budget}_{mini_tag}')),
]
...
if output_mode == "Affordance":
    if _IS_TEST:
        ...
        if not resume:
            if is_stage2:
                load_from = None  # Stage-1 (output_mode="VQA") trains from the base VLM; this branch is not reached
            else:
