...
import numpy as np
def custom_collate_fn(instances, **kwargs):
    base = default_collate_fn(instances, **kwargs)  # keeps core tensors
    data = base["data"]

    extras = {}
    for inst in instances:
        for k, v in inst.items():
            if k in ("input_ids", "labels", "pixel_values", "attention_mask", "position_ids"):
                continue
            extras.setdefault(k, []).append(v)

    for k, vals in extras.items():
        # handle dict-of-np (e.g., cam2imgs/cam2egos)
        if all(isinstance(v, dict) for v in vals):
            stacked = {}
            for subk in vals[0]:
                subvals = [torch.from_numpy(v[subk]) for v in vals]
                stacked[subk] = torch.stack(subvals, 0)
            data[k] = stacked
            continue
        # numpy -> tensor
        vals = [torch.from_numpy(v) if isinstance(v, np.ndarray) else v for v in vals]
        # scalar lists -> tensor
        if all(isinstance(v, (float, int)) for v in vals):
            data[k] = torch.tensor(vals)
        elif all(isinstance(v, torch.Tensor) for v in vals):
            data[k] = torch.stack(vals, 0)
        else:
            data[k] = vals
    return base